...
//    }
    ...
    func test22_Part1_Parsing() throws {
        let step1 = try XCTUnwrap(Day22.rebootStep.parse("on x=-20..26,y=-36..17,z=-47..7"))
        
        XCTAssertEqual(true, step1.state)
        XCTAssertEqual(-20...26, step1.cuboid.x)
        XCTAssertEqual(-36...17, step1.cuboid.y)
        XCTAssertEqual(-47...7, step1.cuboid.z)
    }
    
    func test22_Part1_Example() {
        let exampleSteps = """
        ...
        let onCubeCount = Day22.partOneOptimised(exampleSteps)
        
        XCTAssertEqual(onCubeCount, 39)
    }
    
    func test22_Part1_LargerExample() {
        let exampleSteps = """
        ...
        let onCubeCount = Day22.partOne(exampleSteps)
        
        XCTAssertEqual(onCubeCount, 590784)
    }
    
    func test22_Part1_Solution() throws {
        let result = try XCTUnwrap(Day22.partOne(input_22))
        XCTAssert(result > 0, "Expected non-zero result")
        print("Day 22 (Part 1) answer: \(result)")
        XCTAssertEqual(result, 582644)
    }
    
    func test22_Part1_SolutionOptimized() throws {
        let result = try XCTUnwrap(Day22.partOneOptimised(input_22))
        XCTAssert(result > 0, "Expected non-zero result")
        print("Day 22 (Part 1) answer: \(result)")
        XCTAssertEqual(result, 582644)
    }
    
    func test22_Part2_CuboidIntersections() throws {
        // two cuboids can be:
        // - not intersecting at all
        // - intersecting along a whole edge
        // - intersecting in the middle of an edge
        // - intersecting on a corner
        // - intersecting on two edges
        // - fully intersected (contained within)
        
        let baseCuboid: Day22.Cuboid = (
            x: 0...10,
            y: 0...10,
            z: 0...10
        )
        ...
        XCTAssertNil(
            Day22.intersection(
                between: baseCuboid,
                and: (x: 11...20, y: -10...0, z: 5...15)
            )
        )
        XCTAssertNil(
            Day22.intersection(
                between: baseCuboid,
                and: (x: 0...10, y: 0...10, z: 15...20)
            )
        )
        
        // smaller cube intersecting on one edge
        let cuboid1 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: 8...15, y: 8...15, z: 3...7)
        ))
        XCTAssertEqual(cuboid1.x, 8...10)
        XCTAssertEqual(cuboid1.y, 8...10)
        XCTAssertEqual(cuboid1.z, 3...7)
        
        // larger cube intersecting on one edge
        let cuboid2 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: 8...15, y: 8...15, z: -5...15)
        ))
        XCTAssertEqual(cuboid2.x, 8...10)
        XCTAssertEqual(cuboid2.y, 8...10)
        XCTAssertEqual(cuboid2.z, 0...10)
        
        // intersect on one corner
        let cuboid3 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: 8...15, y: 8...15, z: -5...5)
        ))
        XCTAssertEqual(cuboid3.x, 8...10)
        XCTAssertEqual(cuboid3.y, 8...10)
        XCTAssertEqual(cuboid3.z, 0...5)
        
        // larger intersecting one half of smaller
        let cuboid4 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: -5...15, y: 8...15, z: -5...15)
        ))
        XCTAssertEqual(cuboid4.x, 0...10)
        XCTAssertEqual(cuboid4.y, 8...10)
        XCTAssertEqual(cuboid4.z, 0...10)
        
        // smaller intersecting one face of larger
        let cuboid5 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: 3...7, y: 8...15, z: 3...7)
        ))
        XCTAssertEqual(cuboid5.x, 3...7)
        XCTAssertEqual(cuboid5.y, 8...10)
        XCTAssertEqual(cuboid5.z, 3...7)
        
        // smaller inside larger
        let cuboid6 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: 3...7, y: 3...7, z: 3...7)
        ))
        XCTAssertEqual(cuboid6.x, 3...7)
        XCTAssertEqual(cuboid6.y, 3...7)
        XCTAssertEqual(cuboid6.z, 3...7)
        
        // larger encloses smaller
        let cuboid7 = try XCTUnwrap(Day22.intersection(
            between: baseCuboid,
            and: (x: -5...15, y: -5...15, z: -5...15)
        ))
        XCTAssertEqual(cuboid7.x, 0...10)
        XCTAssertEqual(cuboid7.y, 0...10)
        XCTAssertEqual(cuboid7.z, 0...10)
    }
    
    func test22_Part2_Example() throws {
        let exampleInput = """
        ...
        """
        let result = try XCTUnwrap(Day22.partTwo(exampleInput))
        XCTAssertEqual(result, 2758514936282235)
    }
    
    func test22_Part2_Solution() throws {
        let result = try XCTUnwrap(Day22.partTwo(input_22))
        XCTAssert(result > 0, "Expected non-zero result")
        print("Day 22 (Part 2) answer: \(result)")
        XCTAssertEqual(result, 1263804707062415)
    }
}
